...
(function (global, jQuery, undefined) {
		...
		for (i = 0; i < l; i++) {
			...
			if ((map = props.map || tag).map) {
				if (args.length || props.map) {
					thisMap = tagCtx.map = $extend(tagCtx.map || {
							unmap : map.unmap
						}, props); // Compiled map instance
	...
	function registerStore(storeName, storeSettings) {
		...
		function theStore(name, item, parentTmpl) {
			...
			thisStore = parentTmpl ? parentTmpl[storeNames] = parentTmpl[storeNames] || {}
						...
						(lftPrn
						 ...
						 + (space
							...
							// Insert backspace \b (\x08) as separator for named params, used subsequently by rBuildHash
							 ? (parenDepth && syntaxError(params), named = path, boundName = bound, /*pushBindings(),*/
								'\b' + path + ':')
							 : path
							// path
...
(function (global, $, undefined) {
	...
	function defaultAttr(elem, to, linkGetVal) {
		...
		attr = $viewsSettings.merge[nodeName] || elem.contentEditable === "true" && {
			to : htmlStr,
			from : htmlStr
		};
		return attr
